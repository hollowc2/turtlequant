...
# ///
"""Live dashboard for the TurtleQuant paper/live bot.

Shows open positions (with live edge from last reprice), recent events,
and a running summary of closed positions.
...
def load_positions(state_dir: Path) -> tuple[float, list[dict]]:
    path = state_dir / "turtlequant-positions.json"
    if not path.exists():
        return 1000.0, []
    try:
...
def tail_log(state_dir: Path, n: int = 6) -> list[str]:
    # Try env LOG_FILE first, then common locations
    log_file = os.getenv("LOG_FILE", "")
    candidates = [Path(log_file)] if log_file else []
    candidates += [
        state_dir / "turtlequant-bot.log",
        state_dir.parent / "turtlequant-bot.log",
    ]
    for p in candidates:
        if p.exists():
...
def build_dashboard(state_dir: Path) -> Group:
    ...
    # ── Header ────────────────────────────────────────────────────────────────
    hdr = Text()
    hdr.append("TurtleQuant  │  ", style="bold cyan")
    ...
    header_panel = Panel(hdr, title="[bold blue]TurtleQuant Monitor[/bold blue]", border_style="blue")
    ...
    if not positions:
        ...
        for p in positions:
            ...
            edge_style = "bold green" if edge > 0.05 else ("yellow" if edge > 0 else "red")
            ...
            try:
                ...
                days_left = str(dl) if dl >= 0 else Text("expired", style="red")
            except Exception:
                pass
    ...
    ev_table.add_column("Time (UTC)", style="dim", min_width=19)
    ...
    ev_table.add_column("Model P", justify="right", min_width=8)
    ev_table.add_column("Mkt P", justify="right", min_width=7)
    ev_table.add_column("Edge", justify="right", min_width=7)
    ev_table.add_column("Size $", justify="right", min_width=7)

    recent_events = list(reversed(history[-12:]))
    if not recent_events:
        ...
        for e in recent_events:
            ...
            ev_table.add_row(
                ...
                Text(f"{e.get('model_prob', 0):.3f}"),
                Text(f"{e.get('yes_price', 0):.3f}"),
                Text(f"{edge:+.3f}", style=edge_style),
                Text(f"${e.get('size_usd', 0):.2f}") if ev == "open" else Text("—", style="dim"),
            )

    ev_panel = Panel(ev_table, title="[dim]RECENT EVENTS (last 12)[/dim]", border_style="dim")

    # ── Log tail ───────────────────────────────────────────────────────────────
    renderables: list = [header_panel, pos_panel, ev_panel]
...
def main() -> None:
    parser = argparse.ArgumentParser(description="TurtleQuant bot monitor")
    parser.add_argument("--live", action="store_true", help="Auto-refresh (Ctrl-C to exit)")
    parser.add_argument("--interval", type=int, default=15, metavar="SEC", help="Refresh interval (default: 15)")
    parser.add_argument(
        "--state-dir",
        type=Path,
